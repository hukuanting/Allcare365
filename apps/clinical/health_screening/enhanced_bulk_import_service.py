...
class EnhancedBulkImportService:
    ...
    def process_fhir_import(self, fhir_data: Any, data_format: str = 'json') -> Dict[str, Any]:
        ...
        """
        logger.info(f"Starting FHIR import with data format: {data_format}")
        try:
            # 解析 FHIR 資料
            if data_format == 'json':
                if isinstance(fhir_data, str):
                    try:
                        ...
                        logger.debug("Successfully parsed JSON string into dict.")
                    except json.JSONDecodeError as e:
                        logger.error(f"Invalid JSON format: {str(e)}")
                        raise ValueError(f"Invalid JSON format: {str(e)}")
                else:
                    fhir_json = fhir_data
                    logger.debug("Using provided dict as JSON data.")
            elif data_format == 'xml':
                # 處理 XML 格式 FHIR 資料
                logger.debug("Parsing XML data.")
                fhir_json = self._parse_fhir_xml(fhir_data)
            else:
                logger.error(f"Unsupported FHIR format: {data_format}")
                raise ValueError(f"不支援的 FHIR 格式: {data_format}")
            
            # 驗證 FHIR 資源類型
            if not self._validate_fhir_structure(fhir_json):
                logger.error("Invalid FHIR resource structure.")
                return {
                    'success': False,
            ...
            # 處理 FHIR 資源
            logger.info("FHIR structure validated. Processing resources.")
            result = self._process_fhir_resources(fhir_json)
            logger.info(f"FHIR import finished. Result: {result}")
            
            return result
            
        except Exception as e:
            logger.error(f"FHIR import error: {str(e)}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'processed_count': 0,
    ...
    def _process_fhir_resources(self, fhir_data: Dict[str, Any]) -> Dict[str, Any]:
        """處理 FHIR 資源並匯入資料庫"""
        logger.info("Starting to process FHIR resources.")
        self.processed_count = 0
        self.error_count = 0
        self.errors = []
        created_patients = 0
        ...
        try:
            ...
            if fhir_data.get('resourceType') == 'Bundle':
                logger.info(f"Processing a Bundle with {len(fhir_data.get('entry', []))} entries.")
                ...
                logger.info("Processing a single FHIR resource.")
            ...
            logger.info(f"Found {len(patients)} Patients, {len(observations)} Observations, {len(encounters)} Encounters.")
            ...
            for i, patient_resource in enumerate(patients):
                try:
                    with transaction.atomic():
                        logger.debug(f"Processing Patient resource #{i+1}")
                        ...
                        if patient:
                            ...
                            logger.debug(f"Mapped patient_id '{patient_id}' to Django patient object.")
                                ...
                                for entry in fhir_data.get('entry', []):
                                    if (entry.get('resource', {}).get('resourceType') == 'Patient' and 
                                        ...
                                        if full_url:
                                            ...
                                            logger.debug(f"Mapped fullUrl '{full_url}' to Django patient object.")
                                            # 也添加純 UUID 部分的映射
                                            if 'urn:uuid:' in full_url:
                                                uuid_part = full_url.replace('urn:uuid:', '')
                                                patient_mapping[uuid_part] = patient
                                                logger.debug(f"Mapped uuid '{uuid_part}' to Django patient object.")
                                        break
                            if hasattr(patient, '_created') and patient._created:
                                created_patients += 1
                                logger.info(f"Created new patient with medical_record_number: {patient.medical_record_number}")
                            else:
                                logger.info(f"Found existing patient with medical_record_number: {patient.medical_record_number}")
                    ...
                    logger.error(f"Patient processing error: {str(e)}", exc_info=True)
                    self.errors.append(f"Patient processing error: {str(e)}")
                    self.error_count += 1
            
            logger.info(f"Finished processing Patient resources. {len(patient_mapping)} patients mapped.")
            # 處理觀察結果（生命徵象、實驗室數據）
            for i, obs_resource in enumerate(observations):
                try:
                    with transaction.atomic():
                        logger.debug(f"Processing Observation resource #{i+1}")
                    ...
                    logger.error(f"Observation processing error: {str(e)}", exc_info=True)
                    self.errors.append(f"Observation processing error: {str(e)}")
                    self.error_count += 1
            
            logger.info("Finished processing Observation resources.")
            # 處理就診記錄
            for i, enc_resource in enumerate(encounters):
                try:
                    with transaction.atomic():
                        logger.debug(f"Processing Encounter resource #{i+1}")
                    ...
                    logger.error(f"Encounter processing error: {str(e)}", exc_info=True)
                    self.errors.append(f"Encounter processing error: {str(e)}")
                    self.error_count += 1
            
            logger.info("Finished processing Encounter resources.")
            return {
                'success': True,
            ...
            logger.error(f"FHIR processing error: {str(e)}", exc_info=True)
            return {
                'success': False,
                'error': str(e),
                'processed_count': self.processed_count,
    ...
    def _create_patient_from_fhir(self, patient_resource: Dict[str, Any]) -> Optional[Patient]:
        ...
        logger.debug(f"Attempting to create or update patient from FHIR resource: {patient_resource.get('id')}")
        try:
            # 提取患者 ID
            ...
            if not patient_id:
                ...
                logger.warning(f"Patient resource has no ID. Generated temporary ID: {patient_id}")
            
            # 查找現有患者
            try:
                patient = Patient.objects.get(medical_record_number=str(patient_id))
                logger.info(f"Found existing patient with medical_record_number: {patient_id}")
                # Optionally update patient data here if needed
                return patient
            except Patient.DoesNotExist:
                logger.info(f"No existing patient found with medical_record_number: {patient_id}. Creating new patient.")
                pass
            ...
            logger.debug(f"Prepared patient data for creation: {patient_data}")
            ...
            if created:
                logger.info(f"Successfully created new patient with medical_record_number: {patient_id}")
                patient._created = True
            else:
                logger.info(f"Patient with medical_record_number {patient_id} already existed. Using existing patient.")
                patient._created = False

            return patient
            
        except Exception as e:
            logger.error(f"FHIR patient creation error: {str(e)}", exc_info=True)
            return None
    
    def _process_observation_resource(self, obs_resource: Dict[str, Any], patient_mapping: Dict[str, Patient]):
        """處理 FHIR Observation 資源"""
        logger.debug(f"Processing Observation with ID: {obs_resource.get('id')}")
        try:
            # 找到對應的患者 - 支援多種引用格式
            subject_ref = obs_resource.get('subject', {}).get('reference', '')
            patient = None
            logger.debug(f"Looking for patient with reference: {subject_ref}")
            ...
            if not patient and '/' in subject_ref:
                ...
                if patient:
                    logger.debug(f"Found patient by splitting reference: {patient_id}")
            ...
            if not patient and subject_ref.startswith('urn:uuid:'):
                ...
                if patient:
                    logger.debug(f"Found patient by UUID part: {uuid_part}")

            if not patient:
                logger.warning(f"Patient not found for observation with subject reference: {subject_ref}")
                raise ValueError(f"Patient not found for observation: {subject_ref}")
            
            logger.debug(f"Found patient: {patient.medical_record_number}")
            # 獲取或創建健檢記錄
            ...
            if loinc_code:
                logger.debug(f"Found LOINC code: {loinc_code}")
                self._map_observation_to_model(screening, loinc_code, value, obs_resource)
            else:
                logger.warning(f"No LOINC code found in Observation: {obs_resource.get('id')}")
    ...
    def _get_or_create_screening_for_patient(self, patient: Patient, obs_resource: Dict[str, Any]) -> HealthScreening:
        ...
        logger.debug(f"Using screening date: {screening_date}")

        # 查找當天的健檢記錄
        screening, created = HealthScreening.objects.get_or_create(
            patient=patient,
            screening_date=screening_date,
            defaults={'screening_type': 'fhir_observation'}
        )
        
        if created:
            logger.info(f"Created new HealthScreening for patient {patient.medical_record_number} on {screening_date}")
        else:
            logger.debug(f"Found existing HealthScreening for patient {patient.medical_record_number} on {screening_date}")
    ...
    def _get_loinc_code(self, code_obj: Dict[str, Any]) -> Optional[str]:
        ...
        for coding in codings:
            if coding.get('system') == 'http://loinc.org':
                logger.debug(f"Found LOINC code: {coding.get('code')}")
                return coding.get('code')
        logger.warning(f"No LOINC code found in code object: {code_obj}")
        return None
    
    def _map_observation_to_model(self, screening: HealthScreening, loinc_code: str, value: Any, obs_resource: Dict[str, Any]):
        """將觀察結果映射到資料模型"""
        logger.debug(f"Mapping LOINC code {loinc_code} to model.")
        # LOINC 代碼映射表
        loinc_mappings = {
            # 生命徵象
        ...
        if not field_type:
            logger.warning(f"No mapping found for LOINC code: {loinc_code}")
            return
        
        logger.debug(f"Mapped LOINC code {loinc_code} to field type: {field_type}")
        # 特殊處理：血壓面板（包含收縮壓和舒張壓組件）
        if field_type == 'blood_pressure_panel':
            logger.debug("Processing blood pressure panel components.")
        ...
        logger.debug(f"Extracted numeric value: {numeric_value}")
        # 根據類型存儲到相應的模型
    ...
    def _save_vital_signs(self, screening: HealthScreening, field_type: str, value: float):
        """保存生命徵象資料"""
        logger.debug(f"Saving vital sign '{field_type}' with value {value} for screening {screening.id}")
        ...
        if model_field:
            ...
            vital_signs.save()
            logger.info(f"Saved {model_field} = {value} to VitalSigns for screening {screening.id}")
    
    def _save_laboratory_results(self, screening: HealthScreening, field_type: str, value: float):
        """保存實驗室檢查資料"""
        logger.debug(f"Saving lab result '{field_type}' with value {value} for screening {screening.id}")
        ...
        if model_field:
            ...
            lab_results.save()
            logger.info(f"Saved {model_field} = {value} to LaboratoryResults for screening {screening.id}")
