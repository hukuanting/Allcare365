...
class BulkImportService:
    ...
    def _process_fhir_bundle_native(self, bundle: Dict[str, Any]) -> Dict[str, Any]:
        """
        Native FHIR Bundle Processor
        1. Persists raw FHIR JSON to FHIRResource model (Compliance)
        2. Shreds data into Django models for legacy app compatibility (Practicality)
        """
        self.processed_count = 0
        self.error_count = 0
        self.errors = []
        ...
        # 1. 識別並索引 Patient 資源 (id -> Patient Model)
        patient_map = {} # FHIR ID -> Django Patient Object
        entries = bundle.get('entry', [])
        
        # Pass 1: Handle Patients first
        for entry in entries:
            resource = entry.get('resource', {})
            if resource.get('resourceType') == 'Patient':
                try:
                    p = self._ingest_fhir_patient(resource)
                    if p:
                        patient_map[resource.get('id')] = p
                        patient_map[entry.get('fullUrl')] = p # Also map fullUrl
                        created_resources += 1
                except Exception as e:
                    self.error_count += 1
                    self.errors.append(f"Patient Error: {str(e)}")

        # Pass 2: Handle Clinical Data (Observations, Conditions, etc.)
        for entry in entries:
            resource = entry.get('resource', {})
            r_type = resource.get('resourceType')
            
            if r_type == 'Patient': continue # Already done
            ...
            try:
                ...
                # Link to Patient
                patient = self._resolve_patient_reference(resource, patient_map)
                
                if patient:
                    if r_type == 'Observation':
                        self._ingest_fhir_observation(resource, patient)
                    elif r_type == 'Condition':
                        self._ingest_fhir_condition(resource, patient)
                    # Add more types as needed (Immunization, Procedure, etc.)
                    self.processed_count += 1
            except Exception as e:
                 # Log but don't stop
                 logger.warning(f"Resource ingestion warning ({r_type}): {str(e)}")

        return {
            'success': True,
            'processed_count': self.processed_count,
            'error_count': self.error_count,
            'total_resources': created_resources,
            'errors': self.errors
        }

    def _persist_fhir_resource(self, resource: Dict[str, Any]):
        """Save raw FHIR JSON to FHIRResource model"""
        r_type = resource.get('resourceType')
        r_id = resource.get('id')
        if not r_type or not r_id: return
    ...
    def _ingest_fhir_patient(self, resource: Dict[str, Any]) -> Optional[Patient]:
        """Ingest FHIR Patient to Django Patient Model"""
        # Persist Raw
        self._persist_fhir_resource(resource)
        
        # Extract Identifiers
        identifiers = resource.get('identifier', [])
        mrn = next((i.get('value') for i in identifiers if i.get('system') == 'http://hospital.smarthealthit.org'), resource.get('id'))
        
        # Extract Name
        name_entry = resource.get('name', [{}])[0]
        first = name_entry.get('given', [''])[0] if isinstance(name_entry.get('given'), list) else name_entry.get('given', '')
        last = name_entry.get('family', '')
        
        # Extract Gender
        gender_map = {'male': 'M', 'female': 'F', 'other': 'U', 'unknown': 'U'}
        sex = gender_map.get(resource.get('gender', 'unknown').lower(), 'U')
        
        dob = resource.get('birthDate')
        
        # Create/Update Django Model
    ...
    def _resolve_patient_reference(self, resource: Dict[str, Any], patient_map: Dict) -> Optional[Patient]:
        """Find the Django Patient object for a resource"""
        subject = resource.get('subject', {}) or resource.get('patient', {})
        ref = subject.get('reference', '')
        
        # 1. Try internal map (for Bundle transactions)
        if ref in patient_map: return patient_map[ref]
        
        # 2. Try ID lookup (e.g. "Patient/123" -> "123")
        if '/' in ref:
            p_id = ref.split('/')[-1]
            if p_id in patient_map: return patient_map[p_id]
            # Try DB lookup
            try:
                return Patient.objects.get(medical_record_number=p_id)
            except Patient.DoesNotExist:
                # Fallback: Try by PK if MRN fails
                try:
                     return Patient.objects.get(id=p_id) # Assuming UUID match
                except:
                     pass
        return None

    def _ingest_fhir_observation(self, resource: Dict[str, Any], patient: Patient):
        """Map FHIR Observation to VitalSigns or LabResults"""
        ...
        # Date
        effective_dt = resource.get('effectiveDateTime') or timezone.now()
        if isinstance(effective_dt, str):
            # Simple truncate to date for screening grouping
            screening_date = effective_dt[:10]
        else:
            screening_date = effective_dt.date()

        # Find or Create Screening context
        screening, _ = HealthScreening.objects.get_or_create(
            patient=patient,
            screening_date=screening_date,
            defaults={'screening_type': 'fhir_import'}
        )

        # LOINC Mapping (Simplified)
        # Vital Signs
        if code == '85354-9' or code == '55284-4': # BP
             components = resource.get('component', [])
             systolic = next((c.get('valueQuantity', {}).get('value') for c in components if '8480-6' in str(c)), None)
             diastolic = next((c.get('valueQuantity', {}).get('value') for c in components if '8462-4' in str(c)), None)
             ...
             VitalSigns.objects.update_or_create(
                 health_screening=screening,
                 defaults={'body_temperature': value}
             )
        elif code == '8867-4': # Heart rate
             VitalSigns.objects.update_or_create(
                 health_screening=screening,
                 defaults={'heart_rate': value}
             )
        elif code == '29463-7': # Weight
             VitalSigns.objects.update_or_create(
                 health_screening=screening,
                 defaults={'body_weight': value}
             )
        elif code == '8302-2': # Height
             VitalSigns.objects.update_or_create(
                 health_screening=screening,
                 defaults={'body_height': value}
             )
        
        # Labs
        elif code in ['2345-7', '2339-0']: # Glucose
            LaboratoryResults.objects.update_or_create(
                health_screening=screening,
                test_name='Glucose',
                defaults={'value_result': str(value), 'result_unit': unit}
            )
    ...
    def _ingest_fhir_condition(self, resource: Dict[str, Any], patient: Patient):
        """Map FHIR Condition to MedicalHistory/Problem"""
        # Simplistic mapping
        code_text = resource.get('code', {}).get('text') or resource.get('code', {}).get('coding', [{}])[0].get('display')
    ...
    def _create_vital_signs(self, screening: HealthScreening, vital_data: Dict[str, Any]):
        ...
        try:
            vital_signs_data = {
                'apps.clinical.health_screening': screening
            }
